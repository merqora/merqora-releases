...
object SupabaseClient {
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    val client by lazy {
        android.util.Log.d("SupabaseClient", "â±ï¸ Initializing Supabase client (lazy)...")
        ...
        android.util.Log.d("SupabaseClient", "âœ… Supabase initialized in ${elapsed}ms")
        
        // Inicializar repositorios que dependen del cliente
        HandshakeRepository.initialize(supabase)
        android.util.Log.d("SupabaseClient", "âœ… HandshakeRepository initialized")
        
        ReputationRepository.initialize(supabase)
        android.util.Log.d("SupabaseClient", "âœ… ReputationRepository initialized")
    ...
    fun channel(topic: String, config: io.github.jan.supabase.realtime.RealtimeChannelBuilder.() -> Unit = {}): io.github.jan.supabase.realtime.RealtimeChannel {
        return client.channel(topic, config)
    }
}
