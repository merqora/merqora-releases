...
object MediaOptimizer {
    ...
    fun optimize(bitmap: Bitmap, mediaType: MediaType): OptimizationResult {
        ...
        Log.i(TAG, "═══ Optimizing ${mediaType.description} ═══")
        ...
        val resized = if (targetW < originalW || targetH < originalH) {
            Log.i(TAG, "  Resize: ${originalW}×${originalH} → ${targetW}×${targetH} (Lanczos3)")
        ...
        Log.i(TAG, "  Complexity: %.2f → Quality: %d (range %d-%d)".format(
        ...
        Log.i(TAG, "  Size: ${formatBytes(originalSizeEstimate)} → ${formatBytes(bytes.size.toLong())} " +
