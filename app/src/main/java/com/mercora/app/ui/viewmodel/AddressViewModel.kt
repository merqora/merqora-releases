package com.mercora.app.ui.viewmodel
...
import com.mercora.app.data.remote.MapboxService
import com.mercora.app.data.repository.AddressRepository
import com.mercora.app.engine.AddressEngine
...
class AddressViewModel @Inject constructor(
    ...
    /**
     * Reverse geocoding (coordenadas â†’ texto)
     */
    ...
    fun deleteAddress(addressId: String, onResult: ((Boolean, String?) -> Unit)? = null) {
        viewModelScope.launch {
            addressRepository.deleteAddress(addressId).fold(
                ...
                onFailure = { error ->
                    val msg = error.message ?: "Error al eliminar"
                    _events.emit(AddressEvent.Error(msg))
    ...
    fun loadAddressForEdit(address: Address) {
        _uiState.value = AddressUiState(
            label = address.label,
    ...
    fun updateAddress(addressId: String, userId: String) {
        viewModelScope.launch {
            Log.d(TAG, "updateAddress called for id=$addressId userId=$userId")

            if (userId.isBlank()) {
                _events.emit(AddressEvent.Error("Error: No se pudo identificar tu usuario."))
                return@launch
            }
            ...
            val updates = mapOf<String, Any>(
                ...
                "street_number" to (state.streetNumber.takeIf { it.isNotBlank() } ?: ""),
                "apartment" to (state.apartment.takeIf { it.isNotBlank() } ?: ""),
                "floor" to (state.floor.takeIf { it.isNotBlank() } ?: ""),
                "neighborhood" to (state.neighborhood.takeIf { it.isNotBlank() } ?: ""),
                "city" to state.city,
                "state_province" to (state.stateProvince.takeIf { it.isNotBlank() } ?: ""),
                "postal_code" to (state.postalCode.takeIf { it.isNotBlank() } ?: ""),
                "country" to state.country.ifBlank { "Uruguay" },
                "country_code" to state.countryCode.ifBlank { "UY" },
                "latitude" to state.latitude,
                "longitude" to state.longitude,
                "confidence_score" to state.confidenceScore,
                "source" to effectiveSource.name.lowercase(),
                "status" to state.addressStatus.name.lowercase(),
                "is_default" to state.isDefault,
                "delivery_instructions" to (state.deliveryInstructions.takeIf { it.isNotBlank() } ?: ""),
                "reference_point" to (state.referencePoint.takeIf { it.isNotBlank() } ?: "")
            )

            addressRepository.updateAddress(addressId, updates).fold(
                onSuccess = { address ->
                    Log.d(TAG, "âœ“ Address updated: id=${address.id}")
                    addressRepository.getAddresses(userId).fold(
                        onSuccess = { freshAddresses ->
                            _addresses.value = freshAddresses
                        },
                        onFailure = {
                            _addresses.value = _addresses.value.map { if (it.id == addressId) address else it }
                        }
                    )
                    _uiState.update { it.copy(isSaving = false) }
                    _events.emit(AddressEvent.AddressSaved(address))
